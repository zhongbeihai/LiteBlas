...
void DGEMM_blislab::compute(const Mat& A, const Mat& B, Mat& C) {
    int m = A.rows();
    int k = A.cols();
    int n = B.cols();
    
    bl_dgemm(m, n, k, A.data(), k, B.data(), n, C.data(), n);
}

string DGEMM_blislab::name() {
    return "blislab";
}

void DGEMM_blislab::bl_dgemm(
        int    m,
        int    n,
        int    k,
        const double *XA,
...
//
void DGEMM_blislab::bl_dgemm_ukr( int    kc,
                                  int    mr,
                                  int    nr,
                                  const double *a,
                                  const double *b,
...
void DGEMM_blislab::bl_macro_kernel(
        int    ib,
        int    jb,
        int    pb,
        const double * packA,
